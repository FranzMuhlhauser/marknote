...
| `dialog:save` | Guarda archivo (con diálogo si no hay ruta). Devuelve `{ ok, path?, error? }`, o `null` si el usuario cancela; un fallo de escritura se informa al renderer (no se lanza). Las rutas elegidas en el diálogo nativo se autorizan implícitamente (igual que `dialog:open`); `assertFileAllowed` solo valida rutas que llegan del renderer |
...
|---|---|---|
| v1.0.2 | 2026-08-28 | **Corrección**: 🔧 Código inline corrupto — al abrir archivos Markdown con código inline (palabras entre comillas invertidas como `código`), los placeholders internos de los preprocesadores se filtraban al editor mostrando caracteres extraños como `CB9`. Causa: los null bytes (`\x00`) usados como marcadores temporales se corrompían durante el procesamiento de markdown-it. Solución: reemplazar null bytes por caracteres Unicode del área de uso privado (`\uE000`–`\uE003`) que sobreviven al pipeline completo de preprocesamiento |
| v1.0.1 | 2026-08-23 | **Nuevo**: 📷 Recuperación de fotos y videos — si el archivo de una imagen o video se pierde (por ejemplo, si borras la carpeta de recursos de tu nota), ya no queda roto para siempre: se muestra un aviso con «Reemplazar…» y «Reintentar», y al guardar la app te avisa si algo sigue faltando · 📋 Pegar imágenes directo — ahora puedes pegar capturas de pantalla o imágenes copiadas desde el Explorador de Windows · **Correcciones**: 🔢 Salida de las listas — al escribir una lista (numerada o con viñetas), presionar Enter dos veces ahora sale siempre de la lista; antes, a veces, seguía creando elementos sin fin |
...
| 5 | Errores de guardado (pérdida de datos) | `dialog:save`/`file:write` devuelven `{ ok, path?, error? }` en vez de lanzar. Solo se marca `modified:false` si la escritura tuvo éxito; toast con el error en caso contrario. Los flujos de cierre (`closeTab`/`closeOthers`/`closeAll`/`closeRight`) **abortan si el guardado falla** — la pestaña no se cierra si su contenido no llegó a disco. `beforeunload` no sale de la app si un guardado falla. **(Fix 2026-08-12)** rutas elegidas en el diálogo nativo se autorizan implícitamente — antes `assertFileAllowed` bloqueaba el 1er guardado de archivos nuevos fuera del workspace y el `filePath` nunca se vinculaba a la pestaña (cada Ctrl+S volvía a abrir el diálogo). `saveAsDoc` ahora fuerza el diálogo (`performSave(..., forceDialog=true)`). | `src/main/index.ts`, `src/renderer/src/hooks/useTabs.ts`, `src/renderer/src/hooks/useKeyboardShortcuts.ts`, `src/renderer/src/env.d.ts` | ✅ |
...
### Código Inline Corrupto (v1.0.2)
- **Síntoma**: Al abrir archivos Markdown con código inline (palabras entre comillas invertidas como `código`), el editor mostraba caracteres extraños como `CB9`, `CB10`, `CB11` en lugar del texto original.
- **Causa raíz**: Las funciones `preprocessTaskLists` y `preprocessMath` en `src/renderer/src/utils/markdown.ts` usaban null bytes (`\x00CB`, `\x00MP`) como placeholders temporales para proteger el código inline mientras procesaban listas de tareas y fórmulas matemáticas. Los null bytes se corrompían o eliminaban durante el procesamiento de markdown-it, causando que la restauración final fallara — los placeholders se filtraban al HTML como texto visible.
- **Fix**: Reemplazar null bytes por caracteres Unicode del área de uso privado (U+E000–U+E003) que son válidos y sobreviven al pipeline completo de preprocesamiento → markdown-it → DOMPurify → TipTap. Cambio en `src/renderer/src/utils/markdown.ts`: `\x00CB{n}\x00` → `\uE000{n}\uE001` y `\x00MP{n}\x00` → `\uE002{n}\uE003`.
